...
{
	name = _T("IMX519");
	sensorType = Sensor_IMX519;
    sid_len = 11;

}

//-------------------------------------------------------------------------------------------------
int IMX519::do_prog_otp(int page, int addr, const void *data, int len)
{
   return SET_ERROR(SENSORDRIVER_ERROR_NOTSUPPORT);
}

int IMX519::do_read_otp(int page, int addr, void *data, int len)
{
   return SET_ERROR(SENSORDRIVER_ERROR_NOTSUPPORT);
}
//-----------------------------------------------------------------------------
int IMX519::do_get_sid(uint8_t *id)
{
...
BOOL IMX519::GetSensorId(__out CString &strSensorId)
{
...
int IMX519::wb_writeback(uint8_t *regs, int len)
{
