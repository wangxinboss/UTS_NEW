...
//-------------------------------------------------------------------------------------------------
int IMX386::do_prog_otp(int page, int addr, const void *data, int len)
{
   return SET_ERROR(SENSORDRIVER_ERROR_NOTSUPPORT);
}

int IMX386::do_read_otp(int page, int addr, void *data, int len)
{
   return SET_ERROR(SENSORDRIVER_ERROR_NOTSUPPORT);
}
//-----------------------------------------------------------------------------
int IMX386::do_get_sid(uint8_t *id)
{
	if (do_read_otp(0x13, 0x0A20, (char*)id, sid_len) < 0)
	{
		return -1;
	}
	return 0;
...
BOOL IMX386::GetSensorId(__out CString &strSensorId)
{
...
int IMX386::wb_writeback(uint8_t *regs, int len)
{
	// 	dev->write_sensor(0x5032, regs, len);
	// 
	dev->write_sensor(0x033b, 0x00);
