...
{
	name = _T("S5K2L7");
	sensorType = Sensor_S5K2L7;
    sid_len = 5;

}

//-------------------------------------------------------------------------------------------------
int S5K2L7::do_prog_otp(int page, int addr, const void *data, int len)
{
   return SET_ERROR(SENSORDRIVER_ERROR_NOTSUPPORT);
}

int S5K2L7::do_read_otp(int page, int addr, void *data, int len)
{
   return SET_ERROR(SENSORDRIVER_ERROR_NOTSUPPORT);
}
//-----------------------------------------------------------------------------
int S5K2L7::do_get_sid(uint8_t *id)
{
...
BOOL S5K2L7::GetSensorId(__out CString &strSensorId)
{
...
int S5K2L7::wb_writeback(uint8_t *regs, int len)
{
	char en = 1;
