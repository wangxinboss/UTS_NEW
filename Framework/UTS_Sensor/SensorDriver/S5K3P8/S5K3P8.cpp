...
{
	name = _T("S5K3P8");
	sensorType = Sensor_S5K3P8;
    sid_len = 16;

}

//-------------------------------------------------------------------------------------------------
int S5K3P8::do_prog_otp(int page, int addr, const void *data, int len)
{
   return SET_ERROR(SENSORDRIVER_ERROR_NOTSUPPORT);
}

int S5K3P8::do_read_otp(int page, int addr, void *data, int len)
{
   return SET_ERROR(SENSORDRIVER_ERROR_NOTSUPPORT);
}
//-----------------------------------------------------------------------------

int S5K3P8::do_get_sid(uint8_t *id)
{
	/*
...
BOOL S5K3P8::GetSensorId(__out CString &strSensorId)
{
...
int S5K3P8::wb_writeback(uint8_t *regs, int len)
{
