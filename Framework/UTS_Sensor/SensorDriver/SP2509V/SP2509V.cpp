...
{
	name = _T("SP2509V");
	sensorType = Sensor_SP2509V;
    sid_len = 5;
}
//-------------------------------------------------------------------------------------------------
int SP2509V::do_prog_otp(int page, int addr, const void *data, int len)
{
    return SET_ERROR(SENSORDRIVER_ERROR_NOTSUPPORT);
}
//-------------------------------------------------------------------------------------------------
int SP2509V::do_read_otp(int page, int addr, void *data, int len)
{
   return SET_ERROR(SENSORDRIVER_ERROR_NOTSUPPORT);
}
//-------------------------------------------------------------------------------------------------
//int SP2509V::set_exposure(int e)
//	{
...
//	}
//	int SP2509V::get_exposure_settings(int e, std::map<int, int> &regs)
//	{
//		int i2c = 0;
