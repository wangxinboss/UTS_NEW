...
unsigned short Accuracy(float ACCURACY, unsigned short RADIUS, unsigned short DEGSTEP, unsigned short WAIT_MSEC1, unsigned short WAIT_MSEC2, unsigned short WAIT_MSEC3)
{
	...
	xRadius = 0.220881F * fabsf(fix2float(xGyrogain)) * fabsf(fix2float(xGLenz)) * 4;
	yRadius = 0.220881F * fabsf(fix2float(yGyrogain)) * fabsf(fix2float(yGLenz)) * 4;
	...
	// Circle check
	xpos = xRadius * cos(0.0);
	ypos = yRadius * sin(0.0);
