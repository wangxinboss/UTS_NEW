...
UINT_8 FlashUpdateF40( UINT_8 flag, DOWNLOAD_TBL* ptr )
{
...
//	UINT_8	UserMagicCode[ ptr->SizeMagicCode ] ;
	...
	UserMagicCode = (UINT_8 *)malloc( ptr->SizeMagicCode );
...
//--------------------------------------------------------------------------------
	...
	IOWrite32A( SYSDSP_DSPDIV,				0x00000001 ) ;					// DSPDIV
...
//--------------------------------------------------------------------------------
	if( flag ) {
		...
		ans = UnlockCodeSet() ;								// Unlock Code Set
		if ( ans != 0 ) {
			free( UserMagicCode ) ;		/* release */
			return (ans);							// Unlock Code Set
		}

		IOWrite32A( FLASHROM_F40_ADR,		0x00000000 ) ;		// ADR[31:17]=0000h, ADR[16]=1			// ADR[15:0]=0000h
		IOWrite32A( FLASHROM_F40_CMD,		0x00000005 ) ;		// Chip Erase Start
		WitTim( 13 ) ;
		UlCnt=0;
		do {
			if( UlCnt++ > 100 ) {
...
//--------------------------------------------------------------------------------
	...
	IOWrite32A( FLASHROM_F40_CMD,			0x00000004 ) ;			// Sector Erase Start
	WitTim( 5 ) ;
	UlCnt=0;
	do {
		if( UlCnt++ > 100 ) {
...
//--------------------------------------------------------------------------------
	...
	if ( ans != 0 ) {
		free( UserMagicCode ) ;		/* release */
		return (ans);				//既にErrorが発生していたらWPB=LOWにしてから抜ける
	}
...
//--------------------------------------------------------------------------------
	...
	SiWrkVl0 = 0;
TRACE("sizeof(CcFromCodeF40) %x(%d)\n",sizeof(ptr->FromCode), sizeof(ptr->FromCode)  ) ;
