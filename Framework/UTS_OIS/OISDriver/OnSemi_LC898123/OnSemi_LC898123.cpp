...
int OIS_ON123::LoadFW()
{
	...
	result = FlashDownloadF40_HOLITECH(0 , 0x14 , 0x10);
...
int OIS_ON123::ReadFWver(INT32 *fwver)
{
	int error = UTS::OIS_ERROR_NO;
	int result =  0;
...
int OIS_ON123::GetGyroX(INT32 *X )
{
	RamRead32A(0x258,X);
...
int OIS_ON123::GetGyroY(INT32 *Y )
{
	RamRead32A(0x25C,Y);
...
int OIS_ON123::HallCheck(float *fxMaxAcc,float *fyMaxAcc,float *fxLimit,float *fyLimit,void *fxy_raw_data)
{
	unsigned short ret;
	int error = UTS::OIS_ERROR_NO;

	/*
	ACCURACY  = 3.0    (超過3um 計次 ， 實際看客戶訂幾um )
	RADIUS = 0x64        (範例是 100um  ， 但這次用的馬達行程是 120 um  [0x78] ， 看客戶要測到的行程 ) 
	DEGSTEP = 0x5       (360 度， 每5 度測一次 總共72 點 ，看貴司或客人要測幾點  )
	WAIT_MSEC1 = 0x1F4   (如果行程測到 120um 這個delay time 恐受影響 需要拉長時間 要try )
	WAIT_MSEC2 = 0x20    (測點跟測點的距離會影響這個delay time ， 測點越多，測點間距離越近，delay time 越短)
	WAIT_MSEC3 = 0x04 

	*/
	ret = HallCheck_ON123(3.0,0x64,0x5,0x1F4,0x20,0x04,
		                  fxMaxAcc,fyMaxAcc,fxLimit,fyLimit,fxy_raw_data);
	if(ret !=0)
	{
		error = UTS::OIS_ERROR_HallCheck;
	}

	return (SET_ERROR(error));
}

int OIS_ON123::ErCrossTalkData()
{
	int error = UTS::OIS_ERROR_NO;

	ErCalData(HLLN_CALB_FLG | CROS_TALK_FLG );
