...
//------------------------------------------------------------------------------------------------
	...
	if (UsSts == 0) {
		...
		UsReadVal = abs(long((UlBufDat[ LN_POS7 ] >> 16) - (UlBufDat[ LN_POS1 ] >> 16))) ;
		flDistanceX = ((float)UsReadVal) / 10.0f ;

		// effective range check
TRACE("DISTANCE (X, Y) pixel = (%04X", UsReadVal );

		UsReadVal = abs(long((UlBufDat[ LN_POS7 ] & 0xFFFF) - (UlBufDat[ LN_POS1 ] & 0xFFFF))) ;
