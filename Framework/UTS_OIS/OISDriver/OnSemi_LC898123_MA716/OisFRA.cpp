...
TRACE("CALC gain 0x%x / 0x%x\n", PtFRAMes->UllCumulAdd1, PtFRAMes->UllCumulAdd2 );
	SfTempGain	= ( float )PtFRAMes->UllCumulAdd1 / ( float )PtFRAMes->UllCumulAdd2 ;
	StFRAParam.SfGain[ UcMesCnt ]	= 20.0F * log10f( SfTempGain ) ;
TRACE("CALC gain %f (%fdB)\n", SfTempGain, StFRAParam.SfGain[ UcMesCnt ] );
...
TRACE("CALC phase %f (%fdeg)\n", SfTempPhase, StFRAParam.SfPhase[ UcMesCnt ] );
...
TRACE("MeasureFilter IIR1 a1=b1=%08X\n", UlSampleNum);
...
TRACE("MeasureFilter IIR2 a1=b1=%08X\n", UlSampleNum);
	...
	RamWrite32A ( MeasureFilterA_Coeff_c2, UlSampleNum ) ;
	RamWrite32A ( MeasureFilterB_Coeff_c2, UlSampleNum ) ;
TRACE("MeasureFilter A c=%08X", UlSampleNum);
	...
	RamWrite32A ( MeasureFilterA_Coeff_b1, UlSampleNum ) ;
	RamWrite32A ( MeasureFilterB_Coeff_b1, UlSampleNum ) ;
TRACE(", a=%08X, b=%08X\n", UlSampleNum, UlSampleNum);

	// Calculate coefficient a & b for HPF
	UlSampleNum	= ( 0x7FFFFFFF - UlReadVal ) ;
	RamWrite32A ( MeasureFilterA_Coeff_a2, UlSampleNum ) ;
	RamWrite32A ( MeasureFilterB_Coeff_a2, UlSampleNum ) ;
TRACE("MeasureFilter B a=%08X", UlSampleNum);
...
TRACE("UsFsCount = %d\n", UlReadVal );
	...
	StFRAParam.StMesRslt.SfPhaseAvg	= fmod(float(StFRAParam.StMesRslt.SfPhaseAvg), float(180.0)) ;
...
#if !LPF_ENA
		if( i == 0 ) {
TRACE("WitTim = %d\n",  (int)( 8000 / StFRAParam.StHostCom.SfFrqCom.SfFltVal) );
