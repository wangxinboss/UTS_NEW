...
TRACE("TneCen\nTneOff_3Bit\n");
		...
		UcTofRst	= SUCCESS ;		//Offsetをざっくり取り?いた?とBiasを先ず変化させてみる。?間短縮
	}
...
UINT_32	TneOff_3Bit( UnDwdVal	StTneVal, UINT_8	UcTneAxs , UINT_8 UcPtpMod )
{
	...
	UINT_32	UlDacDat, UlCurDac ;						// 3bit DACへセットする変?
	UINT_8	i, index ;
...
//--------------------------------------
	if( UcTneAxs == X_DIR ) {									// X軸Offset調整
		RamWrite32A( StCaliData_UiHallOffset_X , UlDacDat ) ;	// X軸Offset値を一?変?へコピー

		RamWrite32A( CMD_IO_ADR_ACCESS , VGAVREF ) ;
		RamRead32A(  CMD_IO_DAT_ACCESS , &UlCurDac ) ;			// Offset DACデータRead
		UlDacDat	= ( UlCurDac & 0x000F0F00) | UlDacDat ;		// Y軸データを保?しX軸の調整値を上?き
...
TRACE("Set Offset DAC Data UlCenVal= %08X\n", (UINT_32)UlDacDat);
		...
		RamWrite32A( StCaliData_UiHallOffset_Y , UlDacDat ) ;	// Y軸Offset値を一?変?へコピー

		RamWrite32A( CMD_IO_ADR_ACCESS , VGAVREF ) ;
		RamRead32A(  CMD_IO_DAT_ACCESS , &UlCurDac ) ;			// Offset DACデータRead
		UlDacDat = ( UlCurDac & 0x000F000F ) | ( UlDacDat << 8 ) ;	// X軸データを保?しY軸の調整値を上?き
...
#define 	LOOP_NUM		2463			// 20.019kHz/0.130kHz*16times
#define 	LOOP_FREQ		0x00D4CA33		// 	130Hz  = Freq * 80000000h / Fs
...
TRACE("VAL(X,Y,Z) pos = \t%08xh\t%08xh\t%08xh\t%d \n",(INT_32)SlMeasureRetValueX ,(INT_32)SlMeasureRetValueY,(INT_32)SlMeasureRetValueZ, ucposture ) ;
		...
		case 0:	/* g = +Y */
			if( SlMeasureRetValueY < (INT_32)(POSTURETH_P<<16) ){
				UlRsltSts = EXE_ERR ;
TRACE(" POS0 [ERROR] \t%08xh < %08xh\n",(INT_32)(abs(SlMeasureRetValueY)) , (INT_32)(POSTURETH_P<<16)) ;
			}else{
...
TRACE("POS0(X,Y,Z) st = \t%08xh\t%08xh\t%08xh\t%08xh \n",(INT_32)StPosOff.StPos.Pos[0][0] ,(INT_32)StPosOff.StPos.Pos[0][1],(INT_32)StPosOff.StPos.Pos[0][2],(INT_32)StPosOff.UlAclOfSt ) ;
				}
			}
			break;
		case 1:	/* g = -Y */
			if( SlMeasureRetValueY > (INT_32)(POSTURETH_M<<16) ){
				UlRsltSts = EXE_ERR ;
TRACE(" POS1 [ERROR] \t%08xh > %08xh\n",(INT_32)SlMeasureRetValueY , (INT_32)(POSTURETH_M<<16) ) ;
			}else{
				if( abs(SlMeasureRetValueX) > ZEROG_MRGN_XY )									UlRsltSts |= EXE_GXADJ ;
				if( abs( (INT_32)(ACCL_SENS << 16) - abs(SlMeasureRetValueY)) > ZEROG_MRGN_XY )	UlRsltSts |= EXE_GYADJ ;
				if( abs(SlMeasureRetValueZ) > ZEROG_MRGN_Z )									UlRsltSts |= EXE_GZADJ ;
...
TRACE("POS1(X,Y,Z) st = \t%08xh\t%08xh\t%08xh\t%08xh \n",(INT_32)StPosOff.StPos.Pos[1][0] ,(INT_32)StPosOff.StPos.Pos[1][1],(INT_32)StPosOff.StPos.Pos[1][2],(INT_32)StPosOff.UlAclOfSt ) ;
				}
			}
			break;
		case 2:	/* g = +X */
			if( SlMeasureRetValueX < (INT_32)(POSTURETH_P<<16) ){
				UlRsltSts = EXE_ERR ;
TRACE(" POS2 [ERROR] \t%08xh < %08xh\n",(INT_32)(abs(SlMeasureRetValueX)) , (INT_32)(POSTURETH_P<<16) ) ;
			}else{
...
TRACE("POS2(X,Y,Z) st = \t%08xh\t%08xh\t%08xh\t%08xh \n",(INT_32)StPosOff.StPos.Pos[2][0] ,(INT_32)StPosOff.StPos.Pos[2][1],(INT_32)StPosOff.StPos.Pos[2][2],(INT_32)StPosOff.UlAclOfSt ) ;
				}
			}
			break;
		case 3:	/* g = -X */
			if( SlMeasureRetValueX > (INT_32)(POSTURETH_M<<16) ){
				UlRsltSts = EXE_ERR ;
TRACE(" POS3 [ERROR] \t%08xh > %08xh\n",(INT_32)SlMeasureRetValueX , (INT_32)(POSTURETH_M<<16) ) ;
			}else{
				if( abs( (INT_32)(ACCL_SENS << 16) - abs(SlMeasureRetValueX)) > ZEROG_MRGN_XY )	UlRsltSts |= EXE_GXADJ ;
				if( abs(SlMeasureRetValueY) > ZEROG_MRGN_XY )									UlRsltSts |= EXE_GYADJ ;
				if( abs(SlMeasureRetValueZ) > ZEROG_MRGN_Z )									UlRsltSts |= EXE_GZADJ ;
...
TRACE("POS3(X,Y,Z) st = \t%08xh\t%08xh\t%08xh\t%08xh \n",(INT_32)StPosOff.StPos.Pos[3][0] ,(INT_32)StPosOff.StPos.Pos[3][1],(INT_32)StPosOff.StPos.Pos[3][2],(INT_32)StPosOff.UlAclOfSt ) ;
				}
			}
			break;
		case 4:	/* g = +Z */
...
TRACE("POS4(X,Y,Z) st = \t%08xh\t%08xh\t%08xh\t%08xh \n",(INT_32)StPosOff.StPos.Pos[4][0] ,(INT_32)StPosOff.StPos.Pos[4][1],(INT_32)StPosOff.StPos.Pos[4][2],(INT_32)StPosOff.UlAclOfSt ) ;
		...
		case 5:	/* g = -Z */
			if( SlMeasureRetValueZ > (INT_32)(POSTURETH_M<<16) ){
				UlRsltSts = EXE_ERR ;
TRACE(" POS5 [ERROR] \t%08xh > %08xh\n",(INT_32)SlMeasureRetValueZ , (INT_32)(POSTURETH_M<<16) ) ;
			}else{
				if( abs(SlMeasureRetValueX) > ZEROG_MRGN_XY )									UlRsltSts |= EXE_GXADJ ;
				if( abs(SlMeasureRetValueY) > ZEROG_MRGN_XY )									UlRsltSts |= EXE_GYADJ ;
				if( abs( (INT_32)(ACCL_SENS << 16) - abs(SlMeasureRetValueZ)) > ZEROG_MRGN_Z )	UlRsltSts |= EXE_GZADJ ;
...
TRACE("POS5(X,Y,Z) st = \t%08xh\t%08xh\t%08xh\t%08xh \n",(INT_32)StPosOff.StPos.Pos[5][0] ,(INT_32)StPosOff.StPos.Pos[5][1],(INT_32)StPosOff.StPos.Pos[5][2],(INT_32)StPosOff.UlAclOfSt ) ;
				}
			}
			break;
...
#endif
TRACE("POS0(X,Y,Z) = \t%08xh\t%08xh\t%08xh \n",(INT_32)StPosOff.StPos.Pos[0][0] ,(INT_32)StPosOff.StPos.Pos[0][1],(INT_32)StPosOff.StPos.Pos[0][2] ) ;
TRACE("POS1(X,Y,Z) = \t%08xh\t%08xh\t%08xh \n",(INT_32)StPosOff.StPos.Pos[1][0] ,(INT_32)StPosOff.StPos.Pos[1][1],(INT_32)StPosOff.StPos.Pos[1][2] ) ;
TRACE("POS2(X,Y,Z) = \t%08xh\t%08xh\t%08xh \n",(INT_32)StPosOff.StPos.Pos[2][0] ,(INT_32)StPosOff.StPos.Pos[2][1],(INT_32)StPosOff.StPos.Pos[2][2] ) ;
TRACE("POS3(X,Y,Z) = \t%08xh\t%08xh\t%08xh \n",(INT_32)StPosOff.StPos.Pos[3][0] ,(INT_32)StPosOff.StPos.Pos[3][1],(INT_32)StPosOff.StPos.Pos[3][2] ) ;
TRACE("POS4(X,Y,Z) = \t%08xh\t%08xh\t%08xh \n",(INT_32)StPosOff.StPos.Pos[4][0] ,(INT_32)StPosOff.StPos.Pos[4][1],(INT_32)StPosOff.StPos.Pos[4][2] ) ;
TRACE("POS5(X,Y,Z) = \t%08xh\t%08xh\t%08xh \n",(INT_32)StPosOff.StPos.Pos[5][0] ,(INT_32)StPosOff.StPos.Pos[5][1],(INT_32)StPosOff.StPos.Pos[5][2] ) ;
TRACE("POS_STATUS = \t%08xh\n",(INT_32)StPosOff.UlAclOfSt ) ;
#endif //DEBUG
...
#ifdef DEBUG
TRACE(" MATRIX = \t[%08x] [%08x] [%08x] \n",   (INT_32)mtrx[0] , (INT_32)mtrx[1] , (INT_32)mtrx[2]) ;
TRACE("          \t[%08x] [%08x] [%08x] \n",   (INT_32)mtrx[3] , (INT_32)mtrx[4] , (INT_32)mtrx[5]) ;
TRACE("          \t[%08x] [%08x] [%08x] \n\n", (INT_32)mtrx[6] , (INT_32)mtrx[7] , (INT_32)mtrx[8]) ;
TRACE(" detA = \t%d.%01d%01d%01d%01d%01d \n", (INT_32)detA  , (INT_32)( detA * 10) - ((INT_32)detA)*10 , (INT_32)( detA * 100) - ((INT_32)(detA*10))*10
					, (INT_32)( detA * 1000) - ((INT_32)(detA*100))*10 , (INT_32)( detA * 10000) - ((INT_32)(detA)*1000)*10 , (INT_32)( detA * 100000) - ((INT_32)(detA)*10000)*10 ) ;
TRACE(" IMTRIX = \t[%08x] [%08x] [%08x] \n",   (INT_32)imtrx[0] , (INT_32)imtrx[1] , (INT_32)imtrx[2]) ;
TRACE("          \t[%08x] [%08x] [%08x] \n",   (INT_32)imtrx[3] , (INT_32)imtrx[4] , (INT_32)imtrx[5]) ;
TRACE("          \t[%08x] [%08x] [%08x] \n\n", (INT_32)imtrx[6] , (INT_32)imtrx[7] , (INT_32)imtrx[8]) ;
#endif
			}else{
				UlRsltSts = EXE_ERR ;
			}
...
// History			: First edition
//********************************************************************************
UINT_8	RtnCen( UINT_32	UcCmdPar )
{
...
void	SetSinWavePara( UINT_8 UcTableVal ,  UINT_8 UcMethodVal )
{
	...
	if( UcMethodVal == CIRCWAVE) {
		RamWrite32A( SinWave_Phase	,	0x00000000 ) ;		// 正弦波の位?量
		RamWrite32A( CosWave_Phase 	,	0x20000000 );		// 正弦波の位?量
	}else{
		RamWrite32A( SinWave_Phase	,	0x00000000 ) ;		// 正弦波の位?量
		RamWrite32A( CosWave_Phase 	,	0x00000000 );		// 正弦波の位?量
	}


	if( UlFreqDat == 0xFFFFFFFF )			/* Sine波?止 */
	{
		RamWrite32A( SinWave_Offset		,	0x00000000 ) ;									// 発生周波?のオフセットを設定
		RamWrite32A( SinWave_Phase		,	0x00000000 ) ;									// 正弦波の位?量
//		RamWrite32A( SinWave_Gain		,	0x00000000 ) ;									// 発生周波?のアッテネータ(?期値は0[dB])
//		SetTransDataAdr( SinWave_OutAddr	,	 (UINT_32)SinWave_Output );			// 出力先アド?ス

		RamWrite32A( CosWave_Offset		,	0x00000000 );									// 発生周波?のオフセットを設定
		RamWrite32A( CosWave_Phase 		,	0x00000000 );									// 正弦波の位?量
//		RamWrite32A( CosWave_Gain 		,	0x00000000 );									// 発生周波?のアッテネータ(?期値はCut)
//		SetTransDataAdr( CosWave_OutAddr	,	 (UINT_32)CosWave_Output );			// 出力先アド?ス

		RamWrite32A( SinWaveC_Regsiter	,	0x00000000 ) ;									// Sine Wave Stop
		SetTransDataAdr( SinWave_OutAddr	,	0x00000000 ) ;		// 出力先アド?ス
		SetTransDataAdr( CosWave_OutAddr	,	0x00000000 );		// 出力先アド?ス
		RamWrite32A( HALL_RAM_HXOFF1		,	0x00000000 ) ;				// DelayRam Clear
		RamWrite32A( HALL_RAM_HYOFF1		,	0x00000000 ) ;				// DelayRam Clear
	}
	else
	{
		RamWrite32A( SinWave_Offset		,	UlFreqDat ) ;									// 発生周波?のオフセットを設定
		RamWrite32A( CosWave_Offset		,	UlFreqDat );									// 発生周波?のオフセットを設定

		RamWrite32A( SinWaveC_Regsiter	,	0x00000001 ) ;									// Sine Wave Start
		SetTransDataAdr( SinWave_OutAddr	,	(UINT_32)HALL_RAM_HXOFF1 ) ;		// 出力先アド?ス
		SetTransDataAdr( CosWave_OutAddr	,	 (UINT_32)HALL_RAM_HYOFF1 );		// 出力先アド?ス
...
//	GetDir( &outX, &outY );		// Get direction and driver port of actuator
	...
	if ( UcPos ) {
		...
		SsOff = (INT_16)(SsOff / sqrt(2.0));		// for circle limit(root2)
	}
...
void	SetSinWavGenInt( void )
{
	...
	RamWrite32A( SinWave_Offset		,	0x00000000 ) ;		// 発生周波?のオフセットを設定
	RamWrite32A( SinWave_Phase		,	0x60000000 ) ;		// 正弦波の位?量
	RamWrite32A( SinWave_Gain		,	0x00000000 ) ;		// 発生周波?のアッテネータ(?期値は0[dB])
//	RamWrite32A( SinWave_Gain		,	0x7FFFFFFF ) ;		// 発生周波?のアッテネータ(?期値はCut)
//	SetTransDataAdr( SinWave_OutAddr	,	(UINT_32)SinWave_Output ) ;		// ?期値の出力先アド?スは、自分の??バ

	RamWrite32A( CosWave_Offset		,	0x00000000 );		// 発生周波?のオフセットを設定
	RamWrite32A( CosWave_Phase 		,	0x00000000 );		// 正弦波の位?量
	RamWrite32A( CosWave_Gain 		,	0x00000000 );		// 発生周波?のアッテネータ(?期値はCut)
//	RamWrite32A( CosWave_Gain 		,	0x7FFFFFFF );		// 発生周波?のアッテネータ(?期値は0[dB])
//	SetTransDataAdr( CosWave_OutAddr	,	(UINT_32)CosWave_Output );		// ?期値の出力先アド?スは、自分の??バ
...
//------------------------------------------------------------------------------------------------
	...
	if (UsSts == 0) {
		...
		UsReadVal = abs(int((UlBufDat[ LN_POS7 ] >> 16) - (UlBufDat[ LN_POS1 ] >> 16))) ;
		flDistanceX = ((float)UsReadVal) / 10.0f ;

		// effective range check
TRACE("DISTANCE (X, Y) pixel = (%04X", UsReadVal );

		UsReadVal = abs(int((UlBufDat[ LN_POS7 ] & 0xFFFF) - (UlBufDat[ LN_POS1 ] & 0xFFFF))) ;
...
//********************************************************************************
// Function Name 	: WrADOOffsetData
// Retun Value		: SUCCESS or FAILURE
// Argment Value	: NON
// Explanation		: Flash Write ADO Offset data Function
// History			: First edition 									2017.11.20
//********************************************************************************
UINT_8	WrADOOffsetData( void )
{
	UINT_32	UlReadVal ;
	UINT_32	UiChkSum1,	UiChkSum2 ;
	UINT_32	UlSrvStat,	UlOisStat ;
...
//------------------------------------------------------------------------------------------------
		RamRead32A(  HALL_RAM_HXOFF , &UlReadVal ) ;
		UlBufDat[ MECHA_CENTER_X ] 	= UlReadVal;							// OIS gyro gain X
		RamRead32A(  HALL_RAM_HYOFF , &UlReadVal ) ;
		UlBufDat[ MECHA_CENTER_Y ] 	= UlReadVal;							// OIS gyro gain Y
//------------------------------------------------------------------------------------------------
// Write gyro calibration data
//------------------------------------------------------------------------------------------------
		WriteCalDataF40( UlBufDat, &UiChkSum1 );							// NVR2へ調整値をwriteしCheckSumを計算
