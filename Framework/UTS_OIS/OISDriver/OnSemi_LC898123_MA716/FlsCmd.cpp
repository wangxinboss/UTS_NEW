...
#if SELECT_ACTUATOR == 0
 #include	"FromCode_05_00.h"
#else
...
//#define BURST_LENGTH ( 32*5 )			// 160 必ず5の倍?で設定すること。最大160Byteまで
#define BURST_LENGTH ( 8*5 ) 			// 40 必ず5の倍?で設定すること。最大160Byteまで
...
#define FLASH_ACCESS_SIZE		32		//2の乗?で設定。MAXは32
...
// History			: First edition
//********************************************************************************
UINT_8 FlashUpdateF40(void)
{
	// ChipEraseしない
	return FlashUpdateF40ex( 0 );
}

UINT_8 FlashUpdateF40ex( UINT_8 chiperase )
{
...
#if SELECT_ACTUATOR == 0
	UINT_8 UserMagicCode[sizeof(CcMagicCodeF40_05_00)];
#else
	UINT_8 UserMagicCode[sizeof(CcMagicCodeF40_05_01)];
#endif
...
//--------------------------------------------------------------------------------
	...
	WitTim( 25 ) ;											// Bootプ?グ??を回すのに25msec必要。
	...
	// Pmem にFlash 関連のコマ?ドを追加
	RamWrite32A( 0x0344, 0x00000014 ) ;						// set Pmem.Lnegth
	SiAdrVal =0x00100000;

	for(UlCnt = 0 ;UlCnt < 25 ; UlCnt++ ){
		RamWrite32A( 0x0340, SiAdrVal ) ;					// アド?スセット
		SiAdrVal+= 0x00000008;								// addressのイ?ク???ト
...
//--------------------------------------------------------------------------------
#if SELECT_ACTUATOR == 0
	FlashBurstWriteF40( CcFromCodeF40_05_00, sizeof(CcFromCodeF40_05_00), 0 );
#else
	FlashBurstWriteF40( CcFromCodeF40_05_01, sizeof(CcFromCodeF40_05_01), 0 );
#endif
...
//--------------------------------------------------------------------------------
#if SELECT_ACTUATOR == 0
	UsChkBlocks = ( sizeof(CcFromCodeF40_05_00) / 160 ) + 1 ;
#else
	UsChkBlocks = ( sizeof(CcFromCodeF40_05_01) / 160 ) + 1 ;
#endif
...
#if SELECT_ACTUATOR == 0
	NcDatVal = CcFromCodeF40_05_00;
#else
	NcDatVal = CcFromCodeF40_05_01;
#endif
	SiWrkVl0 = 0;
#if SELECT_ACTUATOR == 0
	for( UlCnt = 0; UlCnt < sizeof(CcFromCodeF40_05_00) ;UlCnt++){
#else
	for( UlCnt = 0; UlCnt < sizeof(CcFromCodeF40_05_01) ;UlCnt++){
#endif
		SiWrkVl0 += *NcDatVal++;
	}
#if 1
	UsChkBlocks *= 160 ;
#if SELECT_ACTUATOR == 0
	TRACE("sizeof(CcFromCodeF40_05_00) %x(%d)\n",sizeof(CcFromCodeF40_05_00), sizeof(CcFromCodeF40_05_00)  ) ;
#else
	TRACE("sizeof(CcFromCodeF40_05_01) %x(%d)\n",sizeof(CcFromCodeF40_05_01), sizeof(CcFromCodeF40_05_01)  ) ;
#endif
	TRACE("Check sum bytes %x(%d)\n",UsChkBlocks, UsChkBlocks  ) ;
	...
	if( SiWrkVl0 != SiWrkVl1 ){
		TRACE("Verify  error %08x %08x\n",SiWrkVl0, SiWrkVl1  ) ;
		return(0x20);
	}

//--------------------------------------------------------------------------------
// X. CHIP ERASEしない場?は?ーザエ?アのチェックサ?を作成
//--------------------------------------------------------------------------------
...
#if SELECT_ACTUATOR == 0
		for( UlCnt = 0; UlCnt < sizeof(CcMagicCodeF40_05_00) ;UlCnt++) {
			UserMagicCode[UlCnt] = CcMagicCodeF40_05_00[UlCnt];
		}
#else
		for( UlCnt = 0; UlCnt < sizeof(CcMagicCodeF40_05_01) ;UlCnt++) {
			UserMagicCode[UlCnt] = CcMagicCodeF40_05_01[UlCnt];
		}
#endif
...
#if SELECT_ACTUATOR == 0
		NcDatVal = CcMagicCodeF40_05_00;
#else
		NcDatVal = CcMagicCodeF40_05_01;
#endif
	}

#if SELECT_ACTUATOR == 0
//TRACE_DUMP( CcMagicCodeF40_05_00, sizeof(CcMagicCodeF40_05_00) );
//TRACE_DUMP( NcDatVal, sizeof(CcMagicCodeF40_05_00) );
#else
//TRACE_DUMP( CcMagicCodeF40_05_01, sizeof(CcMagicCodeF40_05_01) );
//TRACE_DUMP( NcDatVal, sizeof(CcMagicCodeF40_05_01) );
#endif
...
#if SELECT_ACTUATOR == 0
	FlashBurstWriteF40( NcDatVal, sizeof(CcMagicCodeF40_05_00), 0x00010000 );					// Magic Code Write
#else
	FlashBurstWriteF40( NcDatVal, sizeof(CcMagicCodeF40_05_01), 0x00010000 );					// Magic Code Write
#endif
...
//	ReMapコマ?ドだと、Check Sumを見逃す為ここでは?ーブートをさせます。
...
//	ReMapコマ?ドだと、Check Sumを見逃す為ここでは?ブートをさせます。
	IOWrite32A( SYSDSP_REMAP, 0x00001000 ) ;				// CORE_RST[12], MC_IGNORE2[10] = 1 PRAMSEL[7:6]=01b
	WitTim( 10 ) ;
...
TRACE("%2d(0x%04x), %08x\n", UsSize, UsSize, (UINT_32)(BufDat[ UsSize ]));
