...
namespace UTS
{
    ...
    CString IniFile::ReadValue(LPCTSTR lpAppName, LPCTSTR lpKey, LPCTSTR lpDefault)
    {
        TCHAR szBuffer[1024] = {0};
    ...
    CString IniFile::ReadBlock(LPCTSTR lpStartMark, LPCTSTR lpEndMark)
    {
	...
	//
	BOOL IniFile::INIWrite_File(LPCWSTR lpFile, LPCTSTR lpAppName, LPCTSTR lpKey, int val)
	{
		CString tmp;
		tmp.Format(_T("%d"), val);
		return WritePrivateProfileString(lpAppName, lpKey, tmp, lpFile);
	};
	BOOL IniFile::INIWrite_File(LPCWSTR lpFile, LPCTSTR lpAppName, LPCTSTR lpKey, float val)
	{
		CString tmp;
		tmp.Format(_T("%f"), val);
		return WritePrivateProfileString(lpFile, lpAppName, tmp, lpFile);
	};
	BOOL IniFile::INIWrite_File(LPCWSTR lpFile,LPCTSTR lpAppName, LPCTSTR lpKey, double val)
	{
		CString tmp;
		tmp.Format(_T("%lf"), val);
		return WritePrivateProfileString(lpFile, lpAppName, tmp, lpFile);
	};
	BOOL IniFile::INIWrite_File(LPCWSTR lpFile,LPCTSTR lpAppName, LPCTSTR lpKey, LPCWSTR str)
	{
		return WritePrivateProfileString(lpAppName, lpKey, str, lpFile);
	};
}
