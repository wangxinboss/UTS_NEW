...
#define Thumbnail_Show_Max_W 3000 //Terry: the max image size that shows  in thumbnail
#define Thumbnail_Show_Max_H 3000 //Terry: the max image size that shows  in thumbnail
#define Thumbnail_W            72
#define Thumbnail_H            54
/*
...
METHODDEF(void) my_error_exit (j_common_ptr cinfo)
{
    ...
    USES_CONVERSION;
	MessageBox(NULL, A2T(buffer), _T("JPEG Fatal Error"),MB_ICONSTOP);
...

{
	...
	*/
	...
	//Terry, if jpeg is too large, do not display it

	if( (cinfo.image_width > Thumbnail_Show_Max_W) || (cinfo.image_height > Thumbnail_Show_Max_H ) \
		 || (cinfo.image_width <  Thumbnail_W) || (cinfo.image_height < Thumbnail_H ) ) {
		jpeg_destroy_decompress(&cinfo);
		return NULL;	
	}
