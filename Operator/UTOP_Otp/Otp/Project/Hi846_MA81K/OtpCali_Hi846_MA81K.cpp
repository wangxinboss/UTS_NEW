...
{
	OtpCali_Hi846_MA81K::OtpCali_Hi846_MA81K(BaseDevice *dev):HiOtp(dev)
	{
		otp_type = OTP_TYPE_OTP;
		otp_data_len =  sizeof(OTPData);
		otp_lsc_len = MTK_LSC_LEN_MA81K;
		otp_operator_attr |= OTP_OPERATOR_ATTR_USECACHE;
	}

	int OtpCali_Hi846_MA81K::do_prog_otp()
	{
		//烧录数据测试-------------------------------------------------------------------------------------------------------
	...
	int OtpCali_Hi846_MA81K::get_otp_data_from_sensor(void *args)
	{
	...
	int OtpCali_Hi846_MA81K::get_otp_data_from_db(void *args)
	{
		OTPData *otp = (OTPData*)args;
		...
		otp->lsc.LSCGroup1.lscchecksum = (CheckSum(&otp->lsc.LSCGroup1.lscbuffer, MTK_LSC_LEN_MA81K)) % 255 + 1;
	...
	int OtpCali_Hi846_MA81K::LscCali(void *args)
	{
		return do_Hi846_lsc_cali();
	}

	int OtpCali_Hi846_MA81K::get_minfo_from_db(void *args)
	{
		MINFO *m = (MINFO *)args;
	...
	int OtpCali_Hi846_MA81K::do_check_otp(void *args)
	{
		OTPData *otp = (OTPData*)args;
		...
		//检查LSC的CheckSUM
		uint8_t lsc_checkSum = (CheckSum(&(otp->lsc.LSCGroup1.lscbuffer), MTK_LSC_LEN_MA81K)) % 255 + 1;
