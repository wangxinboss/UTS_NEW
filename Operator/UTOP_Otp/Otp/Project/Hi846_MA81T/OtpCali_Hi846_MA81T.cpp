...
namespace OtpCali_Hi846_MA81T 
{
	OtpCali_Hi846_MA81T::OtpCali_Hi846_MA81T(BaseDevice *dev):HiOtp(dev)
	{
		otp_type = OTP_TYPE_OTP;
		otp_data_len =  sizeof(OTPData);
		otp_lsc_len = MTK_LSC_LEN_MA81T;
		otp_operator_attr |= OTP_OPERATOR_ATTR_USECACHE;
	}

	int OtpCali_Hi846_MA81T::do_prog_otp()
	{
		//烧录数据测试-------------------------------------------------------------------------------------------------------
	...
	int OtpCali_Hi846_MA81T::get_otp_data_from_sensor(void *args)
	{
	...
	int OtpCali_Hi846_MA81T::get_otp_data_from_db(void *args)
	{
		OTPData *otp = (OTPData*)args;
		...
		otp->lsc.LSCGroup1.lscchecksum = (CheckSum(&otp->lsc.LSCGroup1.lscbuffer, MTK_LSC_LEN_MA81T)) % 255 + 1;
		...
		//if (ret < 0) { return ret;}
		if (ret < 0 || wb[0].R <= 0 || wb[0].Gr <= 0 || wb[0].Gb <= 0 || wb[0].B <= 0)
		{
			uts.log.Info(_T("get WB Data error!!!"));
		...
		//计算姿势差--------------------
		//marcro = marcro-otp_param.af.off_mac;
		if(marcro < 0) 
		{
			ret = OTPCALI_ERROR_NODATA;
			return SET_ERROR(ret);
		}
		//inf = inf - otp_param.af.off_inf;
		if(inf < 0)
		{
			ret = OTPCALI_ERROR_NODATA;
			return SET_ERROR(ret);
	...
	int OtpCali_Hi846_MA81T::LscCali(void *args)
	{
		return do_Hi846_lsc_cali();
	}

	int OtpCali_Hi846_MA81T::get_minfo_from_db(void *args)
	{
		MINFO *m = (MINFO *)args;
		m->mInfoGroup1.moudleID = 0x42;
		m->mInfoGroup1.afID = 0x01;
		m->mInfoGroup1.sensorID = 0x01;
		m->mInfoGroup1.lensID = 0x01;
		m->mInfoGroup1.DriverICID = 0x01;
		m->mInfoGroup1.vcmID = 0x01;
	...
	int OtpCali_Hi846_MA81T::do_check_otp(void *args)
	{
		OTPData *otp = (OTPData*)args;
		...
		//检查LSC的CheckSUM
		uint8_t lsc_checkSum = (CheckSum(&(otp->lsc.LSCGroup1.lscbuffer), MTK_LSC_LEN_MA81T)) % 255 + 1;
