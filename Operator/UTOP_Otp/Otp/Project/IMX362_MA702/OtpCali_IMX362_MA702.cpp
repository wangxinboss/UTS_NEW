#include "StdAfx.h"
#include "OtpCali_IMX362_MA702.h"
#include "VCM.h"
#include "algorithm.h"
...
namespace OtpCali_IMX362_MA702 {
	//-------------------------------------------------------------------------------------------------
	OtpCali_IMX362_MA702::OtpCali_IMX362_MA702(BaseDevice *dev) : SonyOtp(dev)
	{
	...
	//-------------------------------------------------------------------------------------------------
	int OtpCali_IMX362_MA702::do_prog_otp()
	{
	...
	//-------------------------------------------------------------------------------------------------
	int OtpCali_IMX362_MA702::get_otp_data_from_sensor(void *args)
	{
	...
	//-------------------------------------------------------------------------------------------------
	int OtpCali_IMX362_MA702::LscCali(void *args)
	{
		return do_qulcomm_lsc_awb_cali();
	}
	//-------------------------------------------------------------------------------------------------

	int OtpCali_IMX362_MA702::do_qualcomm_LSCAWBCali(int v5u_bayer,unsigned char *pRaw10, int w, int h, uint8_t out[],uint8_t awbout[])
	{
	...
	//-------------------------------------------------------------------------------------------------
	int OtpCali_IMX362_MA702::get_minfo_from_db(void *args)
	{
// 		struct MINFO
// 		{	
// 			uint8_t module;
...
// 		};
		...
		m->module = 0x01;	//MA702:0x01, MA704:0x02, MA705:0x03
		SYSTEMTIME stime;
		time_t time;
		...
		m->sensor_type = 0x01;	//0x01: IMX362, 0x02: 2L7SA03	
	...
	int OtpCali_IMX362_MA702::get_otp_data_from_db(void *args)
	{
		OTPData *otp = (OTPData*)args;
...
// 		RELEASE_ARRAY(pdafbuf);
	...
	//-------------------------------------------------------------------------------------------------
	int OtpCali_IMX362_MA702::DCC0Cali(void *args)
	{
		return  get_qulcomm_2pd_REVL();
	}


	int OtpCali_IMX362_MA702::PDAFVerify(void *args)
	{
		return qulcomm_dcc_verify_REL();
	}
	//-------------------------------------------------------------------------------------------------
	int OtpCali_IMX362_MA702:: SPCCali(void *args)
	{
		return SPCCali_2PD();
	}
	
	
	int OtpCali_IMX362_MA702::do_SPCCali(uint16_t *pRaw10,int width, int height, uint8_t out[], int max_len)
	{
		PDAF_QulComm_RevL pdaf;

		pdaf.get_sensor_cfg_imx362();
		return pdaf.SPCCali_2PD(pRaw10,width,height,out);
	}

	int OtpCali_IMX362_MA702::do_DCCCali(uint8_t *SPCTable,uint16_t **imagebuf, 
	...
	bool OtpCali_IMX362_MA702::do_qulcomm_DCCVerify_REV(uint8_t *DCCTable,uint16_t *img)
	{
		PDAF_QulComm_RevL pdaf;

		pdaf.get_sensor_cfg_imx362();
	    //pdaf.DCCVerify_2PD(img,DCCTable);//2018/3/22“Ú±‡“Î≤ªπ˝◊¢ Õ
		return true;
	}
