#include "StdAfx.h"
#include "OtpCali_S5K2L7_MA704.h"
#include "VCM.h"
#include "algorithm.h"
...
namespace OtpCali_S5K2L7_MA704 {
	//-------------------------------------------------------------------------------------------------
	OtpCali_S5K2L7_MA704::OtpCali_S5K2L7_MA704(BaseDevice *dev) : SamsungOtp(dev)
	{
	...
	//-------------------------------------------------------------------------------------------------
	int OtpCali_S5K2L7_MA704::do_prog_otp()
	{
	...
	//-------------------------------------------------------------------------------------------------
	int OtpCali_S5K2L7_MA704::get_otp_data_from_sensor(void *args)
	{
	...
	//-------------------------------------------------------------------------------------------------
	int OtpCali_S5K2L7_MA704::LscCali(void *args)
	{
	...
	int OtpCali_S5K2L7_MA704::do_qualcomm_LSCAWBCali(int v5u_bayer,unsigned char *pRaw10, int w, int h, uint8_t out[],uint8_t awbout[])
	{
	...
	//-------------------------------------------------------------------------------------------------
	int OtpCali_S5K2L7_MA704::get_minfo_from_db(void *args)
	{
// 		struct MINFO
// 		{	
// 			uint8_t module;
...
// 		};
		...
		m->module = 0x02;	//MA702:0x01, MA704:0x02, MA705:0x03
		SYSTEMTIME stime;
		time_t time;
		...
		m->lens = 0x02;			//0x01: 60023A1, 0x02: C5501A
		m->vcm = 0x02;			//0x01: CL-18, 0x02: ATMJ1Z952A
		m->vcmdriver = 0x02;	//0x01: AK7374, 0x02: DW9800W
	...
	int OtpCali_S5K2L7_MA704::get_otp_data_from_db(void *args)
	{
		OTPData *otp = (OTPData*)args;
...
// 		RELEASE_ARRAY(pdafbuf);
	...
	//-------------------------------------------------------------------------------------------------
	int OtpCali_S5K2L7_MA704::DCC0Cali(void *args)
	{
	...
	//-------------------------------------------------------------------------------------------------
	int OtpCali_S5K2L7_MA704::PDAFVerify(void *args)
	{
		return 0;
	}
	//-------------------------------------------------------------------------------------------------
	int OtpCali_S5K2L7_MA704:: SPCCali(void *args)
	{
		return SPCCali_2PD();
	}
	
	
	int OtpCali_S5K2L7_MA704::do_SPCCali(uint16_t *pRaw10,int width, int height, uint8_t out[], int max_len)
	{
		PDAF_QulComm_RevL pdaf;
	...
	int OtpCali_S5K2L7_MA704::do_DCCCali(uint8_t *SPCTable,uint16_t **imagebuf, 
