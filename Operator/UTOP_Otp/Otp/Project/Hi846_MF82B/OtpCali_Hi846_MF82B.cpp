...
namespace OtpCali_Hi846_MF82B 
{
	OtpCali_Hi846_MF82B::OtpCali_Hi846_MF82B(BaseDevice *dev):HiOtp(dev)
	{
		otp_type = OTP_TYPE_OTP;
		otp_data_len =  sizeof(OTPData);
		otp_lsc_len = MTK_LSC_LEN_MF82B;
		otp_operator_attr |= OTP_OPERATOR_ATTR_USECACHE;
	}

	int OtpCali_Hi846_MF82B::do_prog_otp()
	{
		//烧录数据测试-------------------------------------------------------------------------------------------------------
	...
	int OtpCali_Hi846_MF82B::get_otp_data_from_sensor(void *args)
	{
	...
	int OtpCali_Hi846_MF82B::get_otp_data_from_db(void *args)
	{
		OTPData *otp = (OTPData*)args;
		...
		otp->lsc.LSCGroup1.lscchecksum = (CheckSum(&otp->lsc.LSCGroup1.lscbuffer, MTK_LSC_LEN_MF82B)) % 255 + 1;
	...
	int OtpCali_Hi846_MF82B::LscCali(void *args)
	{
		return do_Hi846_lsc_cali();
	}

	int OtpCali_Hi846_MF82B::get_minfo_from_db(void *args)
	{
		MINFO *m = (MINFO *)args;
		m->mInfoGroup1.moudleID = 0x42;
		m->mInfoGroup1.afID = 0x00;
		m->mInfoGroup1.sensorID = 0x04;
	...
	int OtpCali_Hi846_MF82B::do_check_otp(void *args)
	{
		OTPData *otp = (OTPData*)args;

		//检查flag
		if (otp->mInfoFlag != 0x01 || otp->lscFlag != 0x01 || otp->wb_flag != 0x01)
		{
			uts.log.Info(_T("OTP Flag Data error!!!"));
			return OTPCALI_ERROR_NODATA;
		}
		//检查Sensor ID AF ID
		if (otp->mInfo.mInfoGroup1.moudleID != 0x42 || otp->mInfo.mInfoGroup1.afID != 0x00)
		{
		...
		//检查LSC的CheckSUM
		uint8_t lsc_checkSum = (CheckSum(&(otp->lsc.LSCGroup1.lscbuffer), MTK_LSC_LEN_MF82B)) % 255 + 1;
