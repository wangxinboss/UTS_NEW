...
//--- NVM Error ---//
#define NVM_FW    5
#define NVM_DATA_CONSIST    509
#define NVM_WRITE           518
#define NVM_CHECK           515
#define NVM_HALLCal         25
#define NVM_DB_CONNECT      501
#define NVM_BURNED_BEFORE   509
#define NVM_NOINFO          510
#define NVM_LSC             314
#define NVM_WB              409
#define NVM_BEGOLDEN        57
#define AE_TIMEOUT          58
#define ERR_QULCOMMLSC      314
#define ERR_SPC             630
#define ERR_DCC             628
...
enum OtpCaliOpt
{
	...
	OtpCaliOpt_WB2RAW10Cali,
	OtpCaliOpt_WB3RAW10Cali,
	OtpCaliOpt_LSCMTK,
	OtpCaliOpt_CrossTalk,
	OtpCaliOpt_Num
};
...
struct WB_LIGHT_USHORT
{
	unsigned short RG;
	unsigned short BG;
};
struct WB_DATA_LIGHT_USHORT
{
	WB_DATA_USHORT wb;
	WB_DATA_USHORT wb_g;
	WB_LIGHT_USHORT wb_light;
};
...
{
    int g_a, r_g, b_g;
    int r_gr, b_gb, gb_gr, gr_gb;

    double multi;
    double dg_a, dr_g, db_g;
    double dr_gr, db_gb, dgb_gr, dgr_gb;
};
...
struct WB_PARAM
{
	...
	WB_DATA_SHORT raw_after_multi2;
...
struct OTP_PARAM
{
	int group_for_use;
	//------add by wx
	int nWB_Type;//for awb ÈýÖÖÉ«ÎÂ
	int LightConfident_RG;
	int LightConfident_BG;
	//-----------
	int nEEPRomType;
...
public:
	...
	int check_wb_with_data(const WB_DATA_UCHAR *wb);
	...
	int CheckWBLimit_Ushort(WB_DATA_USHORT*wb);
	...
	virtual int do_check_otp(void *args,void *args2) {return SET_ERROR(OTPCALI_ERROR_NO);}
    ...
    int upload_lsc();
	//---------add by wx
	int do_qulcomm_cali(int type);
	int do_mtk_lsc_cali_MTK051017_RGB();

	virtual int LSCMTK_Cail(void *args) {return SET_ERROR(OTPCALI_ERROR_NO);}
	virtual int CrossTalk(void *args) {return SET_ERROR(OTPCALI_ERROR_NO);};
	int do_crosstalk();
	//add by wx for sewen
	int get_wb_from_raw_data(WB_DATA_USHORT *wb, WB_DATA_USHORT *wb_g,WB_LIGHT_USHORT *wb_light,int WBType);
	int get_wb_from_raw_data(WB_DATA_USHORT *wb, WB_DATA_USHORT *wb_g,int WBType);
	virtual int do_qualcomm_Type_Cali(int v5u_bayer,unsigned char *pRaw10, 
		int w, int h, uint8_t out[],int type){return SET_ERROR(OTPCALI_ERROR_NO);}


	int do_Hi846_lsc_cali(BOOL updateFlag = TRUE);
	int do_OV13855_lsc_cali(BOOL updateFlag = TRUE);
    ...
    int get_wb_from_raw_data(WB_DATA_UCHAR *wb, WB_DATA_UCHAR *wb_g);
	int get_wb_from_raw_data(WB_DATA_USHORT *wb, WB_DATA_USHORT *wb_g);
	 //int get_wb_from_raw_data(WB_DATA_USHORT *wb, WB_DATA_USHORT *wb_g, int wbType);
	...
	int check_lsc_qualcommm(uint8_t *module,int lsclength);
	...
	int qulcomm_dcc_verify_2pd_REVL();
	int sony_dcc_verify();
	...
	virtual int do_WriteSPCSetting3(){return SET_ERROR(OTPCALI_ERROR_NO);}
	...
	virtual int do_qulcomm_DCCVerify_REV(uint8_t *DCCTable,uint16_t *img,int& DAC){return TRUE;}
	...
	int SamsungLscCali();
	virtual int do_SamsungLSCCali(uint16_t *pRaw10,int width, int height, char *otp, int &otp_len,char *sram, int &sram_len)
	...
	int doSamsungLSCCali_5E2(uint16_t *pRaw10,int width, int height, 
...
protected:
    ...
    bool is_otp_mem_def();
    virtual int do_dump_otp();
    virtual int do_dump_eeprom();
