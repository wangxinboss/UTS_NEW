...
#include "UTOP_ResolutionNokia.h"
#include "VCM.h"
...
namespace UTS
{
    ...
    BOOL ResolutionOperator::OnPreview(unsigned char *pBmpBuffer, int nWidth, int nHeight, UI_MARK &uiMark)
    {
        ...
        for (size_t i = 0; i < m_param.sfr.vecField.size(); i++)
        {
			...
			//Nokia delta
            singleFieldResult.dSfrDelta[SFR_DELTA_RTLB] = ((m_result.stSfrResult.dSFR[m_param.sfr.vecField[1].vecSfrIndex[1]][SFR_Frq_Hi] -
														   m_result.stSfrResult.dSFR[m_param.sfr.vecField[1].vecSfrIndex[3]][SFR_Frq_Hi])/
													   max(m_result.stSfrResult.dSFR[m_param.sfr.vecField[1].vecSfrIndex[1]][SFR_Frq_Hi], 
														   m_result.stSfrResult.dSFR[m_param.sfr.vecField[1].vecSfrIndex[3]][SFR_Frq_Hi]))*100.00;

			singleFieldResult.dSfrDelta[SFR_DELTA_LTRB] = ((m_result.stSfrResult.dSFR[m_param.sfr.vecField[1].vecSfrIndex[2]][SFR_Frq_Hi] - 
														   m_result.stSfrResult.dSFR[m_param.sfr.vecField[1].vecSfrIndex[0]][SFR_Frq_Hi])/
													   max(m_result.stSfrResult.dSFR[m_param.sfr.vecField[1].vecSfrIndex[2]][SFR_Frq_Hi], 
														   m_result.stSfrResult.dSFR[m_param.sfr.vecField[1].vecSfrIndex[0]][SFR_Frq_Hi]))*100.00;
        ...
        for (size_t i = 0; i < m_param.sfr.vecField.size(); i++)
        {
			...
			// for j
            if (m_result.vecFieldResult[i].dSfrDelta[SFR_DELTA_LTRB] <= m_param.sfr.vecField[i].dHiDeltaSpec ||
				m_result.vecFieldResult[i].dSfrDelta[SFR_DELTA_RTLB] <= m_param.sfr.vecField[i].dHiDeltaSpec)
            {
            ...
            strSfrDelta.Format(_T("SFRFiled%d_HiDelta: %.1f ,%.1f"),
                i, m_result.vecFieldResult[i].dSfrDelta[SFR_DELTA_LTRB]
			     , m_result.vecFieldResult[i].dSfrDelta[SFR_DELTA_RTLB]);
...
//             uiMark.vecUITextRel.push_back(textRel);
    ...
    void ResolutionOperator::GetDataContent(LPCTSTR lpTime, CString &strHeader, CString &strData, CString &strSFCFilter)
    {
        ...
        //-------------------------------------------------------------------------
        // Field
        for (int i = 1; i < m_param.sfr.nFieldCount; i++)
        {
            strHeader.AppendFormat(_T("Filed%d_LTRB,"), i);
            strHeader.AppendFormat(_T("Filed%d_RTLB,"), i);
            
            strData.AppendFormat(_T("%.1f,"), m_result.vecFieldResult[i].dSfrDelta[SFR_DELTA_LTRB]);
            strData.AppendFormat(_T("%.1f,"), m_result.vecFieldResult[i].dSfrDelta[SFR_DELTA_RTLB]);
        }
