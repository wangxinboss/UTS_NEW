...
#include "UTOP_DarkNoise_Alterk.h"
...
namespace UTS
{
    ...
    BOOL DarkNoiseOperator::OnReadSpec()
    {
		...
		uts.dbCof.GetOperatorSingleSpec(strSection, _T("TestROI"), strValue, _T("0,0,1600,1200"), _T("参考ROI(左,上,宽,高)"));
...
// 			}
            ...
            DarkNoise_WholeImage::DarkNoiseAltek(
                pChartROIBmpBuffer,
        ...
        //------------------------------------------------------------------------------
        // 判断规格
        if (m_dStdDevR< m_param.dNoiseSpec.min || m_dStdDevR > m_param.dNoiseSpec.max ||
			m_dStdDevG< m_param.dNoiseSpec.min || m_dStdDevG > m_param.dNoiseSpec.max ||
			m_dStdDevB< m_param.dNoiseSpec.min || m_dStdDevB > m_param.dNoiseSpec.max)
        {
    ...
    void DarkNoiseOperator::GetDataContent(LPCTSTR lpTime, CString &strHeader, CString &strData, CString &strSFCFilter)
    {
        ...
        strData.Format(
			...
			, m_dStdDevR,m_dStdDevG,m_dStdDevB
