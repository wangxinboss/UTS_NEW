...
namespace UTS
{
    ...
    BOOL ImplOperator::OnReadSpec()
    {
        ...
        uts.dbCof.GetOperatorSingleSpec(strSection, _T("strOTPTestOpt"), m_param.strOTPTestOpt, _T("3"), _T("0:ProgOTP, 1:CheckOTP, 2:LSC, 3:WB, 6:LOADLSC ,\
																								 ...
																								 25:GyroCaliNonShaker"));
        uts.dbCof.GetOperatorSingleSpec(strSection, _T("nSensorBlackLevel"), m_param.nSensorBlackLevel, 4, _T("SensorBlackLevel"));
        uts.dbCof.GetOperatorSingleSpec(strSection, _T("nGROUP_FOR_USE"), m_param.nGROUP_FOR_USE, 0, _T("GROUP_FOR_USE"));

		//EEProm
		uts.dbCof.GetOperatorSingleSpec(strSection, _T("nEEPRomType"), m_param.nEEPRomType, 0, _T("nEEPRomType"));
	
		//FPN
		uts.dbCof.GetOperatorSingleSpec(strSection, _T("nFPNSpec"), m_param.dFPNSpec, 3.50, _T("nFPNSpec"));
		...
		//PDAF
		//uts.dbCof.GetOperatorSingleSpec(strSection, _T("nPDAF_Cal_Mode"), pdaf_sony_calmode, 0, _T("nPDAF_Cal_Mode"));
		...
		uts.dbCof.GetOperatorSingleSpec(strSection, _T("nPDAF_Cal_Ini_File"),     m_param.pdaf_param.pdafcal_ini_file,          _T(""), _T("nPDAF_Cal_Ini_File"));
		uts.dbCof.GetOperatorSingleSpec(strSection, _T("nPDAF_CDAF_Type"),        m_param.pdaf_param.pdafverify.cdaf_type,        0,    _T("nPDAF_CDAF_Type"));
		uts.dbCof.GetOperatorSingleSpec(strSection, _T("nPDAF_CDAF_Step"),        m_param.pdaf_param.pdafverify.cdaf_Step,        0,    _T("nPDAF_CDAF_Step"));
		uts.dbCof.GetOperatorSingleSpec(strSection, _T("nPDAF_CDAF_Serchoffset"), m_param.pdaf_param.pdafverify.cdaf_serchoffset, 0,    _T("nPDAF_CDAF_Serchoffset"));
    ...
    BOOL ImplOperator::SetOtpParam()
    {
		...
		//FPN
		 m_pOtp->otp_param.dFPNSpec = m_param.dFPNSpec;
        // WB
		...
		//	uts.log.Info(_T("start write."));
			...
			case OtpCaliOpt_WBRAW10Cali:
				*pnErrorCode = m_pOtp->WBCali_RAW10(pbIsRunning) ;
				break;
			case OtpCaliOpt_SRAMLOADLSC:
				*pnErrorCode = m_pOtp->check_sram_off_lsc(pbIsRunning) ;
				break;
            default:
                *pnErrorCode = uts.errorcode.E_Fail;
                break;
    ...
    {
        vecRegister.clear();
		vecRegister.push_back(_T("OTP_WBLSC_TEST1"));
		vecRegister.push_back(_T("OTP_WBLSC_TEST2"));
        ...
        for (size_t i = 0; i < m_vecOtpTestOpt.size(); i++)
        {
            ...
            }else if (m_vecOtpTestOpt[i] == OtpCaliOpt_LscCali)
			{
				if (0 == _tcscmp(uts.info.strProjectName, _T("MA702"))
				 ||(0 == _tcscmp(uts.info.strProjectName, _T("MA704")))
				 ||(0 == _tcscmp(uts.info.strProjectName, _T("MA705")))
				 ||(0 == _tcscmp(uts.info.strProjectName, _T("MA722")))
				 ||(0 == _tcscmp(uts.info.strProjectName, _T("MF82K"))))
				{
			...
			else if(m_vecOtpTestOpt[i] == OtpCaliOpt_PDAFVerify)
			{
				...
				strHeader += _T("SobelDAC,PDAFInitDAC,VerifyDAC,PDAFVerifyError");
				strData.AppendFormat(
					_T("%d,%d,%d,%.2f,")
					,pdaf->pdafResult.SobelDAC
					,pdaf->pdafResult.PDAFInitDAC
					,pdaf->pdafResult.PDAFVerifyDAC
					,pdaf->pdafResult.PDAFVerifySonyError
					);
			...
			}else if(m_vecOtpTestOpt[i] == OtpCaliOpt_WBRAW10Cali)
			{
				...
				strData.AppendFormat(
					...
					, pwb_param->ratio.dr_gr
					, pwb_param->ratio.db_gb
					, pwb_param->wb_data_c_aftercali2.R
					, pwb_param->wb_data_c_aftercali2.Gr
					, pwb_param->wb_data_c_aftercali2.Gb
					, pwb_param->wb_data_c_aftercali2.B
					, pwb_param->rg_delta
					, pwb_param->bg_delta
					, pwb_param->raw_after_multi2.R
					, pwb_param->raw_after_multi2.Gr
					, pwb_param->raw_after_multi2.Gb
					, pwb_param->raw_after_multi2.B
					, pwb_param->delta_rgVSG
					, pwb_param->delta_bgVSG);
