...
SamsungOtp::SamsungOtp(BaseDevice *dev) : BaseOtp(dev)
{
}


SamsungOtp::~SamsungOtp(void)
{
}

//-------------------------------------------------------------------------------------------------
int SamsungOtp::get_wb_gain(WB_RATIO *ratio, WB_RATIO *ratio_target, WB_GAIN *gain)
{
    get_gain_by_wb_ratio(ratio,ratio_target,0x100,gain);
    return 0;
}

int SamsungOtp::get_wb_cali_data(WB_GAIN *gain, void *out)
{
...
int SamsungOtp::write_reg(char *buf, int len)
{
...
int SamsungOtp::chagetexttohex(char *buf, char *output, int len)
{
    int reg;
    int count = 0;
    char	addr_buf[5];
