#include "StdAfx.h"
#include "OtpCali_IMX499_MA81S.h"
#include "VCM.h"
#include "algorithm.h"
...
namespace OtpCali_IMX499_MA81S {
	//-------------------------------------------------------------------------------------------------
	OtpCali_IMX499_MA81S::OtpCali_IMX499_MA81S(BaseDevice *dev) : SonyOtp(dev)
	{
	...
	//-------------------------------------------------------------------------------------------------
	int OtpCali_IMX499_MA81S::do_prog_otp()
	{
	...
	//-------------------------------------------------------------------------------------------------
	int OtpCali_IMX499_MA81S::get_otp_data_from_sensor(void *args)
	{
	...
	//-------------------------------------------------------------------------------------------------
	int OtpCali_IMX499_MA81S::LscCali(void *args)
	{
		return do_qulcomm_lsc_awb_cali();
	}
	//-------------------------------------------------------------------------------------------------

	int OtpCali_IMX499_MA81S::do_qualcomm_LSCAWBCali(int v5u_bayer,unsigned char *pRaw10, int w, int h, uint8_t out[],uint8_t awbout[])
	{
	...
	//-------------------------------------------------------------------------------------------------
	int OtpCali_IMX499_MA81S::get_minfo_from_db(void *args)
	{
	...
	int OtpCali_IMX499_MA81S::get_otp_data_from_db(void *args)
	{
		OTPData *otp = (OTPData*)args;
...
/*		ret = otpDB->GetOtpByType(uts.otpdb->GetModuleID(T2A(otp_param.wb_param.goldenSampleName)), 
	...
	//-------------------------------------------------------------------------------------------------
	int OtpCali_IMX499_MA81S:: SPCCali(void *args)
	{
		return SONYSPCCali();
	}

	//-------------------------------------------------------------------------------------------------
	int OtpCali_IMX499_MA81S::DCC0Cali(void *args)
	{
		return  do_sony_dcc_cali();
	}

	int OtpCali_IMX499_MA81S::PDAFVerify(void *args)
	{
		return sony_dcc_verify();
	}

	int OtpCali_IMX499_MA81S::do_SONY_SPCCali(unsigned short *pRaw10, short *out)
	{
		USES_CONVERSION;
	...
	int OtpCali_IMX499_MA81S::do_SONY_SPCVerify(unsigned short *pRaw10,int *judge)
	{
		USES_CONVERSION;
	...
	int OtpCali_IMX499_MA81S::do_SONY_DCCCali(int lens1, int lens2, int *pd1, int *pd2,unsigned short *dcc_out, int calc_mode)
	{
		int ret = -1;
		HINSTANCE hdll;
	...
	int OtpCali_IMX499_MA81S::do_SONY_DCCVerify(unsigned short*dcc, int *pd, int *dcc_out, int *pd_out ,int calc_mode)
	{
		int ret = -1;
		HINSTANCE hdll;
	...
	int OtpCali_IMX499_MA81S::do_WriteSPCSetting1()
	{
	...
	int OtpCali_IMX499_MA81S::do_WriteSPCSetting2()
	{
		sensor->dev->write_sensor(0x0101 , 0x00);
		sensor->dev->write_sensor(0x0B00 , 0x00);
		sensor->dev->write_sensor(0x3606 , 0x00);
	...
	int OtpCali_IMX499_MA81S::do_WriteDCCSetting3()
	{
		sensor->dev->write_sensor(0x0101 , 0x00);
		sensor->dev->write_sensor(0x0B00 , 0x00);
	...
	int OtpCali_IMX499_MA81S::do_SPC_writeback(uint8_t *regs)
	{
	...
	int OtpCali_IMX499_MA81S::do_SONY_DCC_stats(int mode)
	{
	...
	int OtpCali_IMX499_MA81S::do_SONY_GetPDTable(int *pdtable)
	{
	...
	int OtpCali_IMX499_MA81S::do_check_otp(void *args)
	{
		OTPData *otp = (OTPData*)args;
