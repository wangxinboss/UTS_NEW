    ...
    BOOL ResolutionOperator::OnTest(BOOL *pbIsRunning, int *pnErrorCode)
    {
		...
		*pnErrorCode = uts.errorcode.E_Pass;
		...
		{				
			...
			for (int j = 0 ; j < 5 ;j++)
			{		
				CurrentXY[0] = TestROI[j].x;
				CurrentXY[1] = TestROI[j].y;
				...
				{
					FocusROI.left = ROIInfo[i].x;
					FocusROI.right = ROIInfo[i].x + ROIInfo[i].width;
					FocusROI.top = m_bufferInfo.nHeight - ROIInfo[i].y  - ROIInfo[i].height;
					//FocusROI.top = ROIInfo[i].y;
					FocusROI.bottom = FocusROI.top + ROIInfo[i].height;
				...
				m_result.dSfr[j]/= iROICount; 
				m_result.dSFRHiThroughValue[j][m_param.nManualThroughFocusFrameCount] = m_result.dSfr[j];
				text.color = COLOR_BLUE;
				text.ptPos = CPoint(rc.rcPos.left, rc.rcPos.bottom);
				text.strText.Format(_T("%.1f"), m_result.dSfr[j]);
