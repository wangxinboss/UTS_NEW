...
namespace UTS
{
    ...
    typedef struct _sfr_plus_operator_param_
    {
		...
		int nCaculateDFOV;
		FOV_PARAM dfov_parm;
		double dfov_cur, dfov_peak, dfov_delta;
		double dsfr_peak;
		
		//OverShoot
	    int nOverShoot;
		MinMax<double> stdfov_deltaSpec;
    ...
    typedef struct _sfr_plus_operator_result_
    {
		...
		double dSFRPreThroughValue[SFR_PLUS_MAX_ROI][Max_VCM_DAC];
    ...
    public:
		...
		BOOL CheckDAC(int _DAC,int &_DBDAC);
